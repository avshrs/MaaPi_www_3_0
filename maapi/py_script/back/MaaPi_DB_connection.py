...
class MaaPiDBConnection(object):
    ...
    def exec_query_select(self,query,name):
        try:
            ...
            raise IOError("Can not open connection with DB {0}".format(Maapi_dbname))
        else:
            table_data_dict={}
            x = conn.cursor()
            try:
    ...
    def columns(self, *args):
        if len(args) != 1:
            raise ValueError(".order_by('column name') should only have a column name")
        self.columns_ = args
        return self
    ...
    def get_table(self,*args):
        if len(args) != 1:
            raise ValueError(".get_table('table name') should only have a table name")
        if self.columns_:
            ...
            try:
               ...
               query += " ORDER BY {0}".format(self.orders_[0])
                ...
                if self.orders_[1]=="asc" or self.orders_[1]=="ASC" or self.orders_[1]=="desc" or self.orders_[1]=="DESC":
                    query += " ORDER BY {0} {1}".format(self.orders_[0],self.orders_[1])
                else:
                    raise ValueError("order_by Second Value should be empty or ASC or DESC but get: '{0}'".format(self.orders_[1]))
        query+=";"
