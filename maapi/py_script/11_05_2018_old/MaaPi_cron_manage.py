...
def run():
    try:
        ...
        x = conn.cursor()
        x.execute(
            "UPDATE maapi_cron SET cron_last_file_exec=now() WHERE cron_file_path ='{0}' and cron_where_exec='{1}'".
            format(sys.argv[0], Maapi_location))
        conn.commit()
        x.execute(
            "SELECT COUNT(*) FROM maapi_cron WHERE cron_enabled=True and cron_where_exec='{0}' ".
            format(Maapi_location))
        counter = x.fetchone()[0]

        x.execute(
            "SELECT id FROM maapi_cron WHERE cron_enabled=True and cron_where_exec='{0}' order by id".
            format(Maapi_location))
        cron_id = x.fetchall()

        x.execute(
            "SELECT cron_minute_id FROM maapi_cron WHERE cron_enabled=True and cron_where_exec='{0}' order by id".
            format(Maapi_location))
        cron_minute_id = x.fetchall()

        x.execute(
            "SELECT cron_minute_on_every_id FROM maapi_cron WHERE cron_enabled=True and cron_where_exec='{0}' order by id".
            format(Maapi_location))
        cron_minute_on_every_id = x.fetchall()

        x.execute(
            "SELECT cron_hour_id FROM maapi_cron WHERE cron_enabled=True and cron_where_exec='{0}' order by id".
            format(Maapi_location))
        cron_hour_id = x.fetchall()

        x.execute(
            "SELECT cron_hour_on_every_id FROM maapi_cron WHERE cron_enabled=True and cron_where_exec='{0}' order by id".
            format(Maapi_location))
        cron_hour_on_every_id = x.fetchall()

        x.execute(
            "SELECT cron_day_id FROM maapi_cron WHERE cron_enabled=True and cron_where_exec='{0}' order by id".
            format(Maapi_location))
        cron_day_id = x.fetchall()

        x.execute(
            "SELECT cron_day_on_every_id FROM maapi_cron WHERE cron_enabled=True and cron_where_exec='{0}' order by id".
            format(Maapi_location))
        cron_day_on_every_id = x.fetchall()

        x.execute(
            "SELECT cron_interpreter FROM maapi_cron WHERE cron_enabled=True and cron_where_exec='{0}' order by id".
            format(Maapi_location))
        cron_interpreter = x.fetchall()

        x.execute(
            "SELECT cron_file_path FROM maapi_cron WHERE cron_enabled=True and cron_where_exec='{0}' order by id".
            format(Maapi_location))
        cron_file_path = x.fetchall()

        x.execute(
            "SELECT cron_comment FROM maapi_cron WHERE cron_enabled=True and cron_where_exec='{0}' order by id".
            format(Maapi_location))
        ...
        for i in xrange(0, counter):
            ...
            if my_cron_len > counter:
                ...
                if var_i == 0:
                    ...
                    var_i = 1
                """ create new crons """
                job = my_cron.new(command="{0} {1}".format(
                    cron_interpreter[i][0], cron_file_path[i][0]))
                job.set_comment("{0}".format(cron_id[i][0]))
                x.execute(
                    "UPDATE maapi_cron set cron_last_exec_date=now() where id={0}".
                    format(cron_id[i][0]))
                conn.commit()
                ...
                try:
                    ...
                    if job.command == "{0} {1}".format(cron_interpreter[i][0],
                                                       cron_file_path[i][0]):
                        qa("command == true")
                    else:
                        job.set_command("{0} {1}".format(
                            cron_interpreter[i][0], cron_file_path[i][0]))
                        qa("command updated")
                        my_cron.write()
                    ...
                    qa("minute - {0} should {1}".format(job.minute,
                                                        cron_minute_id[i][0]))
                    qa("hour   - {0} should {1}".format(job.hour,
                                                        cron_hour_id[i][0]))
                    qa("day    - {0} should {1}".format(job.day,
                                                        cron_day_id[i][0]))
                    ...
                    if cron_minute_id[i][0] is not None:
                        ...
                        if cron_minute_on_every_id[i][0] == 2:
                            if cron_minute_id[i][0] == 1:
                                if job.minute == "*":
                                    qa("e minute== true")
                            elif job.minute == "*/{0}".format(
                                    cron_minute_id[i][0]):
                                qa("e minute== true")
                            else:
                                job.minute.every(cron_minute_id[i][0])
                                qa("e minute updated {0}".format(
                                    job.minute.every(cron_minute_id[i][0])))
                                my_cron.write()
