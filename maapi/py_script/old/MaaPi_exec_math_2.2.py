                ...
                if math_exec_cond_id[i][0] is not None:
                    ...
                    if math_exec_cond_value_min_e[i][0]:
                        ...
                        if dev_value < math_exec_cond_value_min[i][0]:
                            qa('dev_value={0} < math_exec_cond_value_min_e={1}  = {2}'.format(dev_value,math_exec_cond_value_min[i][0], dev_value < math_exec_cond_value_min[i][0]))
                            enable=1
                        else:
                            qa('dev_value={0} < math_exec_cond_value_min_e={1}  = {2}'.format(dev_value,math_exec_cond_value_min[i][0], dev_value < math_exec_cond_value_min[i][0]))
                    if math_exec_cond_value_max_e[i][0]:
                        ...
                        if (dev_value > math_exec_cond_value_max[i][0]):
                            qa('dev_value={0} > math_exec_cond_value_max_e={1}  = {2}'.format(dev_value,math_exec_cond_value_max[i][0], (dev_value < math_exec_cond_value_max[i][0])))
                            enable=1
                            qa('enable =1 ')
                        else:
                            qa('dev_value={0} > math_exec_cond_value_max_e={1}  = {2}'.format(dev_value,math_exec_cond_value_max[i][0], (dev_value < math_exec_cond_value_max[i][0])))
                if enable==1:
                    run_math_expressions(i)
            else:
                run_math_expressions(i)
