...
try:
    ...
    try:
        ...
        for i in xrange(0,counter[0]):
            ...
            """ loop -  get dev Bus id of updateing sensor """
            x.execute("SELECT dev_bus_type_id FROM devices WHERE dev_id='{0}' and dev_status=True".format(update_rom[i][0]))
            ...
            if dev_bus_type_id == bus_id:
                ...
                x.execute("SELECT dev_value FROM devices WHERE dev_id='{0}' ".format(update_rom[i][0]))
                old_value = x.fetchone()[0]

                """ loop - get value of 1 sensor"""
                if math_data_from_1_id[i][0] is not None:
                    x.execute("SELECT dev_value FROM devices WHERE dev_id='{0}' ".format(math_data_from_1_id[i][0]))
                    v1 = x.fetchone()[0]
                    V1 = v1
                else:
                    v1='none'
                ...
                if math_data_from_2_id[i][0] is not None:
                    x.execute("SELECT dev_value FROM devices WHERE dev_id='{0}' ".format(math_data_from_2_id[i][0]))
                    v2=x.fetchone()[0]
                    V2=v2
                else:
                    v2='none'
                ...
                if math_data_from_3_id[i][0] is not None:
                    x.execute("SELECT dev_value FROM devices WHERE dev_id='{0}' ".format(math_data_from_3_id[i][0]))
                    v3 = x.fetchone()[0]
                    V3=v3
                else:
                    v3='none'
                ...
                if math_data_from_4_id[i][0] is not None:
                    x.execute("SELECT dev_value FROM devices WHERE dev_id='{0}' ".format(math_data_from_4_id[i][0]))
                    v4=x.fetchone()[0]
                    V4 = v4
                else:
                     v4='none'
                ...
                try:
                    ...
                    try:
                        x.execute("UPDATE devices SET dev_value_old='{0}' WHERE dev_id='{1}'".format(float(old_value),update_rom[i][0]))
                        qa("execute")
                    ...
                    """ exec - update value """
                    try:
                        x.execute("SELECT dev_rom_id FROM devices WHERE dev_id='{0}' ".format(update_rom[i][0]))
                        dev_rom_id = x.fetchone()[0]
                        x.execute("""INSERT INTO maapi_dev_rom_{0}_values VALUES (default,{1},default,{2})""".format(dev_rom_id.replace("-", "_"), update_rom[i][0],float(value)))
                        qa("execute")
                    except psycopg2.IntegrityError:
                        conn.rollback()
                    ...
                    try:
                        x.execute("UPDATE devices SET dev_value='{0}', dev_last_update=NOW(), dev_read_error='{1}' WHERE dev_id='{2}'".format(float(value),'ok',update_rom[i][0]))
                        qa("execute")
                    except psycopg2.IntegrityError:
                        conn.rollback()
                    ...
                    qa('dupa')
                    x.execute("INSERT INTO logs VALUES (default,default,'ERROR' , 'serv MaaPi_exec_math.py' , 'Math error: {0} |  v1={1}, v2={2}, v4={3}, v4={4}' , 'IntegrityError')".format(math[i][0],v1,v2,v3,v4))
                    conn.commit()
                else:
                    pass
